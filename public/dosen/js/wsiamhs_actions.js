...
function simpanAkun(id, e) {
	if ($$('formAkun').validate()) {
		...
		if (data.passBaru1 == data.passBaru) {
			...
			webix.ajax().post("sopingi/dosen/aksi/" + wSiaMhs.apiKey + "/" + Math.random(), dataKirim, function (response, d, xhr) {
				proses_hide();
...
function simpanMahasiswa(id, e) {
	if ($$('formMahasiswaDetail').validate()) {
		...
		webix.ajax().post("sopingi/mahasiswa/" + aksi + "/" + wSiaMhs.apiKey + "/" + Math.random(), dataKirim, {
...
function updateFoto() {
	...
	if (typeof file_id != "undefined") {
		$$('foto').send(function () {
			$$('fotoMhs').setHTML("<center><img src='foto/" + wSiaMhs.nidnMd5 + ".jpg?" + Math.random() + "' height='140'></center>");
...
function simpanKRS(id, e) {
	if ($$('formKRS').validate()) {
		...
		if (totalSKS <= 24) {
			...
			webix.ajax().post("sopingi/nilai/tambah/" + wSiaMhs.apiKey + "/" + Math.random(), dataKirim, {
				success: function (response, data, xhr) {
					proses_hide();
					hasil = JSON.parse(response);
					if (hasil.berhasil) {
...
function hapusKRS(data) {
	...
	webix.ajax().post("sopingi/nilai/" + aksi + "/" + wSiaMhs.apiKey + "/" + id, dataKirim, {
		success: function (response, data, xhr) {
			proses_hide();
			hasil = JSON.parse(response);
			if (hasil.berhasil) {
...
function unduhKRS() {
	window.open(wSiaMhs.domain + "/dosen/sopingi/krs_pdf/download/" + wSiaMhs.apiKey + "/" + Math.random(), "_blank");
};
...
function khsPDF() {
	...
	window.open(wSiaMhs.domain + "/dosen/sopingi/khs_pdf/download/" + wSiaMhs.apiKey + "/" + id, "_blank");
};

function transkipPDF() {
	window.open(wSiaMhs.domain + "/dosen/sopingi/transkip_pdf/download/" + wSiaMhs.apiKey + "/" + Math.random(), "_blank");
};
...
function simpanPembimbing() {
	if ($$('formPembimbing').validate()) {
		...
		aksi = data.aksi;
		dataKirim = JSON.stringify(data);
		webix.ajax().post("sopingi/pa_aktifitas/" + aksi + "/" + wSiaMhs.apiKey + "/" + Math.random(), dataKirim, {
			success: function (response, data, xhr) {
				proses_hide();
				hasil = JSON.parse(response);
				if (hasil.berhasil) {
...
function hapusPembimbing(data) {
	...
	aksi = data.aksi;
	dataKirim = JSON.stringify(data);
	webix.ajax().post("sopingi/pa_aktifitas/" + aksi + "/" + wSiaMhs.apiKey + "/" + id, dataKirim, {
		success: function (response, data, xhr) {
			proses_hide();
			hasil = JSON.parse(response);
			if (hasil.berhasil) {
...
function handleLinkGoogleResponse(response) {
	if (response.credential) {
		...
		webix.ajax().post("sopingi/profil/link_google/" + wSiaMhs.apiKey + "/" + Math.random(), dataKirim, function (text) {
			proses_hide();
...
window.unlinkGoogleAccount = function () {
	webix.confirm({
		...
		callback: function (result) {
			if (result) {
				proses_tampil();
				webix.ajax().post("sopingi/profil/unlink_google/" + wSiaMhs.apiKey + "/" + Math.random(), {}, function (text) {
					proses_hide();
					var hasil = JSON.parse(text);
					if (hasil.berhasil) {
						webix.message(hasil.pesan);
