...
<head>
	<meta charset="utf-8">
	<title>Mahasiswa Login | POLITEKNIK INDONUSA Surakarta</title>
...
<body class="app_wsiamhs" bgcolor="#244531">
	...
	<script type="text/javascript" charset="utf-8">
		...
		function handleCredentialResponse(response) {
			proses_tampil();
			webix.ajax().post("<?php echo $domain; ?>/mhs/login-google", JSON.stringify({ token: response.credential }), {
				success: function (text, data, xhr) {
				proses_hide();
				var hasil = JSON.parse(text);
				if (hasil.berhasil) {
					webix.storage.session.put('wSiaMhs', { domain: hasil.domain, nipd: hasil.nipd, xid_reg_pd: hasil.xid_reg_pd, nm_pd: hasil.nm_pd, apiKey: hasil.apiKey, nipdMd5: hasil.nipdMd5, mulai_smt: hasil.mulai_smt, email_poltek: hasil.email_poltek });
					$$("formLogin").disable();
					webix.message(hasil.pesan);
					window.location = '<?php echo $domain; ?>/mhs';
				} else {
					webix.alert({
		...
		webix.ui({
			...
			elements: [
				{
					view: "fieldset", label: "Login Mahasiswa", body: {
						rows: [
							{ view: "text", id: "user", name: "user", label: "NIM", labelWidth: 100, placeholder: "Ketik NIM", required: true, invalidMessage: "NIM belum diisi" },
		...
		$$("masuk").attachEvent("onItemClick", function (id, e) {
			if ($$('formLogin').validate()) {
				...
				proses_tampil();
				webix.ajax().post("<?php echo $domain; ?>/mhs/login", dataKirim, {
