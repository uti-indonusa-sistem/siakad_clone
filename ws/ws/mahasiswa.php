	...
	$qryWsMahasiswa="select * from view_mahasiswa_tambah_2020 order by nik asc limit 50 OFFSET $id";
		...
		if ($data) {
			foreach($data as $row){
				...
				if (isset($row->no_tel_rmh) && strlen($row->no_tel_rmh)<9) {
					unset($row->no_tel_rmh);
				}

				if (isset($row->no_hp) && $row->no_hp=="-") {
					unset($row->no_hp);
				}
				...
				if (!isset($dataMahasiswaFeeder['result'])) {
					...
					$update[] = array(
						...
						"data" => array(
							...
							"no_hp" => isset($row->telepon_seluler) ? $row->telepon_seluler : '',
